// UniworkComposer is a fork of Composer for the Uniwork workbench. It keeps the
// real input, attachment, slash command, and submission behavior, while Uniwork
// owns settings and status controls inside the immersive bottom composer.
...
import { Activity, ArrowUp, Check, ChevronUp, CircleDollarSign, CircleGauge, Database, Eye, FileText, Folder, Laptop, Layers, MessageSquare, Plus, Search, Shield, ShieldAlert, ShieldCheck, Square, Trash2, Wallet, X, Zap } from "lucide-react";
...
import { type BalanceInfo, type CollaborationMode, type CommandInfo, type ComposerInsertRequest, type ContextInfo, type DirEntry, type EffortInfo, type HistoryMessage, type JobView, type PromptHistoryEntry, type SessionMeta, type SessionReference, type SlashArgItem, type SlashArgsResult, type TokenMode, type ToolApprovalMode, type WireUsage } from "../lib/types";
import {
...
function formatUniworkTurnCount(turns: number | undefined, t: ReturnType<typeof useI18n>["t"]): string {
  if (typeof turns !== "number" || turns < 0) return "-";
  return t(turns === 1 ? "uniwork.status.turnOne" : "uniwork.status.turnOther", { n: turns });
}

function uniworkAccessLabel(mode: ToolApprovalMode, t: ReturnType<typeof useI18n>["t"]): string {
  if (mode === "yolo") return t("uniwork.status.accessYolo");
  if (mode === "auto") return t("uniwork.status.accessAuto");
  return t("uniwork.status.accessAsk");
}

function UniworkStatusMenuRow({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="modelsw__item uniwork-status-menu__row" role="listitem">
      <span className="uniwork-status-menu__label">
        {icon}
        <span>{label}</span>
      </span>
      <strong>{value}</strong>
...
function compactWorkspaceLabel(workspaceName?: string, workspacePath?: string): string {
  const name = (workspaceName || "").trim();
  if (name) return name;
  const path = (workspacePath || "").trim().replace(/\\/g, "/").replace(/\/+$/, "");
  if (!path) return "";
...
export function UniworkComposer({
  running,
  collaborationMode,
  toolApprovalMode,
  goal,
  ...
  onCycleMode: () => void;
  onSetCollaborationMode: (mode: CollaborationMode) => void | Promise<void>;
  onSetToolApprovalMode: (mode: ToolApprovalMode) => void | Promise<void>;
  onToggleYoloApprovalMode: () => void;
  onClearGoal: () => void | Promise<void>;
  onSwitchModel: (name: string) => void;
  onSetEffort: (level: string) => void;
  onSetTokenMode: (mode: TokenMode) => void | Promise<void>;
  ...
  const submit = async () => {
    ...
    if (!trimmedText && attachments.length === 0 && workspaceRefs.length === 0) {
      if (goalModeOn && !activeGoal) {
        setComposerPrompt(t("uniwork.composer.goalInputRequired"));
        requestAnimationFrame(() => taRef.current?.focus());
      }
      return;
    }
  ...
  const runActivity = retry
    ...
    : running && turnStartAt
      ...
      : null;
  const workspaceLabel = compactWorkspaceLabel(workspaceName, workspacePath) || t("uniwork.composer.workspaceFallback");
  const jobsList = jobs ?? [];
  ...
  const busyLabel = hydrationLabel || runActivity || (running ? t("uniwork.status.running") : t("uniwork.status.ready"));
  const accessLabel = uniworkAccessLabel(toolApprovalMode, t);
  const approvalOptions: Array<{ mode: ToolApprovalMode; icon: ReactNode; title: string; description: string }> = [
    {
      mode: "ask",
      icon: <Shield size={16} />,
      title: t("uniwork.accessPanel.askTitle"),
      description: t("uniwork.accessPanel.askDesc"),
    },
    {
      mode: "auto",
      icon: <ShieldCheck size={16} />,
      title: t("uniwork.accessPanel.autoTitle"),
      description: t("uniwork.accessPanel.autoDesc"),
    },
    {
      mode: "yolo",
      icon: <ShieldAlert size={16} />,
      title: t("uniwork.accessPanel.yoloTitle"),
      description: t("uniwork.accessPanel.yoloDesc"),
    },
  ];
  ...
  return (
    <div
      className={`composer-wrap uniwork-composer-wrap${decisionPending ? " composer-wrap--decision-pending" : ""}`}
    ...
    >
      ...
      <div
        className={`composer-card uniwork-composer-card${composerHeight !== null || composerResizing ? " composer-card--resized" : ""}${composerAutoExpanded ? " composer-card--autosized" : ""}${composerResizing ? " composer-card--resizing" : ""}${running ? " composer-card--running" : ""}`}
        ref={composerCardRef}
        style={composerCardStyle}
      >
        <div className="uniwork-composer__scope-row" aria-label={t("uniwork.composer.context")}>
          <div className="uniwork-composer__context-group">
            <span className="uniwork-composer__context-item">
              <Laptop size={13} />
              <span>{t("uniwork.composer.local")}</span>
            </span>
            <span className="uniwork-composer__context-separator">/</span>
            <span className="uniwork-composer__context-item uniwork-composer__context-item--primary" title={workspacePath || workspaceLabel}>
              <Folder size={13} />
            ...
            <span className="uniwork-composer__context-item">
              <FileText size={13} />
              <span>{t("uniwork.activitySwitcher.uniwork")}</span>
            </span>
          </div>
        </div>
        <button
        ...
        />
        <div
          className={`composer uniwork-composer__input${dragOver ? " composer--dragover" : ""}${disabled || readOnly ? " composer--disabled" : ""}${shellModeActive ? " composer--shell" : ""}`}
          onDrop={onDrop}
        ...
        >
          ...
          <textarea
            ...
            aria-label={t("uniwork.composer.placeholder")}
            value={text}
            onChange={(e) => {
            ...
            style={textareaStyle}
            placeholder={readOnly ? t("uniwork.composer.readOnlyChannel") : disabled ? t("common.loading") : goalModeOn && !activeGoal ? t("uniwork.composer.goalInputPlaceholder") : t("uniwork.composer.placeholder")}
            rows={1}
        ...
        <div className="uniwork-composer__utility-row">
          <div className="uniwork-composer__utility-left">
            <button
              ref={controlsTriggerRef}
              type="button"
              className={controlsMenuOpen ? "uniwork-composer__mode-trigger uniwork-composer__mode-trigger--open" : "uniwork-composer__mode-trigger"}
              onClick={() => {
                setControlsMenuOpen((open) => !open);
                setStatusMenuOpen(false);
              }}
              aria-label={t("uniwork.status.controls")}
            ...
            <Tooltip label={t("uniwork.composer.addContext")}>
              <button
                type="button"
                className="uniwork-composer__icon-button"
                onClick={addContextReference}
                disabled={disabled || readOnly}
                aria-label={t("uniwork.composer.addContext")}
              >
                <Plus size={16} />
              </button>
            </Tooltip>
          ...
          <div className="uniwork-composer__utility-right">
            ...
            <button
              ...
              type="button"
              className={statusMenuOpen ? "uniwork-composer__health-trigger uniwork-composer__health-trigger--open" : "uniwork-composer__health-trigger"}
              onClick={() => {
                setStatusMenuOpen((open) => !open);
                setControlsMenuOpen(false);
              }}
              aria-label={t("uniwork.status.details")}
              title={busyLabel}
              aria-haspopup="dialog"
              aria-expanded={statusMenuOpen}
            >
              <span className={running ? "uniwork-composer__dot uniwork-composer__dot--busy" : "uniwork-composer__dot"} />
            </button>
          </div>
          ...
          <AnchoredPopover
            ...
            className="modelsw__menu modelsw__menu--portal uniwork-controls-menu"
            style={{ minWidth: 300, maxWidth: "min(90vw, 360px)" }}
          >
            <div role="listbox" aria-label={t("uniwork.status.controls")}>
              {approvalOptions.map((option) => {
                const selected = toolApprovalMode === option.mode;
                return (
                  <button
                    key={option.mode}
                    type="button"
                    role="option"
                    className={selected ? "modelsw__item modelsw__item--current uniwork-controls-menu__option uniwork-controls-menu__option--active" : "modelsw__item uniwork-controls-menu__option"}
                    onClick={() => {
                      setControlsMenuOpen(false);
                      void onSetToolApprovalMode(option.mode);
                    }}
                    disabled={controlsDisabled}
                    aria-selected={selected}
                  >
                    <span className="uniwork-controls-menu__option-icon">{option.icon}</span>
                    <span className="modelsw__copy uniwork-controls-menu__option-copy">
                      <span className="modelsw__model uniwork-controls-menu__option-title">{option.title}</span>
                      <span className="uniwork-controls-menu__option-desc">{option.description}</span>
                    </span>
                    {selected && <Check className="modelsw__check uniwork-controls-menu__option-check" size={13} aria-hidden="true" />}
                  </button>
                );
              })}
            </div>
          ...
          <AnchoredPopover
            ...
            className="modelsw__menu modelsw__menu--portal uniwork-status-menu"
            align="end"
            style={{ minWidth: 300, maxWidth: "min(90vw, 360px)" }}
          >
            <div role="status" aria-live="polite" aria-label={t("uniwork.status.details")}>
              <div className="modelsw__group-label uniwork-status-menu__head">
                <span>{t("uniwork.status.details")}</span>
                <strong>{busyLabel}</strong>
              </div>

              <div className="uniwork-status-menu__section" role="list">
                <UniworkStatusMenuRow icon={<Activity size={13} />} label={t("uniwork.status.sessionTurns")} value={formatUniworkTurnCount(sessionTurns, t)} />
                <UniworkStatusMenuRow icon={<Database size={13} />} label={t("uniwork.status.sessionTokens")} value={formatTokenCount(sessionTokens)} />
                <UniworkStatusMenuRow icon={<Zap size={13} />} label={t("uniwork.status.turnTokens")} value={formatTokenCount(statusTurnTokens)} />
              </div>
              <div className="uniwork-status-menu__section" role="list">
                <UniworkStatusMenuRow icon={<CircleGauge size={13} />} label={t("uniwork.status.context")} value={pct !== null ? `${pct}%` : "-"} />
                <UniworkStatusMenuRow icon={<Layers size={13} />} label={t("uniwork.status.compact")} value={compactPct !== null ? `${compactPct}%` : "-"} />
                <UniworkStatusMenuRow icon={<Activity size={13} />} label={t("uniwork.status.cacheTurn")} value={nowPct !== null ? `${nowPct}%` : "-"} />
                <UniworkStatusMenuRow icon={<Activity size={13} />} label={t("uniwork.status.cacheAverage")} value={avgPct !== null ? `${avgPct}%` : "-"} />
              </div>
              <div className="uniwork-status-menu__section" role="list">
                <UniworkStatusMenuRow icon={<CircleDollarSign size={13} />} label={t("uniwork.status.turnCost")} value={turnCostLabel} />
                <UniworkStatusMenuRow icon={<CircleDollarSign size={13} />} label={t("uniwork.status.sessionCost")} value={costLabel} />
                <UniworkStatusMenuRow icon={<Wallet size={13} />} label={t("uniwork.status.balance")} value={balanceLabel} />
              </div>
              <div className="uniwork-status-menu__section" role="list">
                <UniworkStatusMenuRow icon={<Check size={13} />} label={t("uniwork.status.jobs")} value={jobsList.length > 0 ? t("uniwork.status.jobsRunning", { n: jobsList.length }) : t("uniwork.status.noJobs")} />
              </div>
            </div>
          </AnchoredPopover>
        </div>
