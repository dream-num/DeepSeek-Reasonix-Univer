...
The V2 Cowork direction is different. Reasonix should provide a native workspace activity and transcript-first shell for office workers collaborating with an agent on documents, spreadsheets, and slides, while Univer tooling owns document semantics, collab state, worktree operations, and embeddable Univer UI. The current `univer-cli` `origin/dev` branch already has the relevant source-of-truth packages:
...
- Make Cowork feel like an office-native document collaboration workspace rather than a developer preview, terminal, or code review surface.
...
### 2. Cowork is designed for office workers, not developers

Cowork is for users who think in documents, spreadsheets, slides, drafts, reviews, and decisions. The UI should avoid developer-density, terminal styling, raw protocol state, JSON, logs, and CLI-oriented diagnostics in the primary path. Reasonix may keep technical details available for debugging, but the main Cowork surfaces should use office-native copy and task-oriented affordances.
...
- Reuse Code-mode density and technical language. Rejected because Cowork is not the recommended path for coding work; it is the primary path for document, spreadsheet, and slide work.
...
### 3. The Cowork shell stays transcript-first
...
### 4. `@univer/cowork` is the single frontend integration package
...
### 6. Cowork target state follows the gateway contract
...
### 8. Cowork information architecture follows document collaboration

The Cowork shell should organize state around sessions, document targets, drafts, and review decisions:

- Activity switch: Code and Cowork are separate primary modes.
- Cowork sidebar: recent Cowork sessions and document targets should be easier to scan than raw workspace files.
- Transcript: user requests, agent responses, and Cowork milestone blocks remain the collaboration record.
...
4. Implement office-native Cowork shell IA, vocabulary mapping, transcript Cowork Blocks, and worktree action flows.
